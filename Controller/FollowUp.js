...
module.exports = {
    ...
    async GetAllFollowUp(req, res) {
        await FollowUp.find().exec((error, followUp) => {
            if (error) {
            ...
            for (let i = 0; i < followUp.length; i++) {
                followUp[i].createdAt = followUp[i].updatedAt = followUp[i].__v = undefined;
            }
    ...
    async DeleteFollowUpById(req, res) {
        if (req.FollowUp.IsActive) {
            await FollowUp.deleteOne({ _id: req.FollowUp._id }).exec((error, output) => {
                if (error) {
                    return res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
